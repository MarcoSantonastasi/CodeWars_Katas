...
Test.describe("More tests", function(){
  ...
  var rndChr = function(str){ return str[rnd(str.length)] };
  
  Test.assertEquals(isIsogram(lowers+rndChr(lowers)), false)
  Test.assertEquals(isIsogram(lowers+rndChr(uppers)), false)
  
  var shuffle = function(str){return str.split('').sort(function(){return Math.random()<.5}).join('')}
  
  console.log("Random tests");
  for( var t=0; t<9; t++ ){
    var cc = shuffle(lowers), exp = true, chr;
    var len = lowers.length, len = Math.min(3+rnd(len),len), str = cc.slice(0,len);
    if( rnd(99)<50 ){
      chr = rndChr(str);
    ...
    console.log("> "+str+" ?")
    Test.assertEquals(isIsogram(str),exp,!exp? " 2 '"+chr+"' in "+str+" !":"")
  }
})
